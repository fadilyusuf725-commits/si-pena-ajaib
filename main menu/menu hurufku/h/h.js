...
/* letter / progress */
let currentLetter = 'H';
try { localStorage.setItem('lastVisitedFull', 'menu hurufku/h/h.html'); } catch(e){}
...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // H: stroke order - left vertical → right vertical → horizontal (3 strokes)
    if (currentLetter.toUpperCase() === 'H') {
      // Number 1 on left vertical
      tctx.fillText('①', Lx - 60, Cy);
      
      // Number 2 on right vertical
      tctx.fillText('②', Lx + 60, Cy);
      
      // Number 3 on middle horizontal
      tctx.fillText('③', Lx, Cy + 10);
    }

    // h: stroke order - vertical then bump (2 strokes)
    if (currentLetter.toLowerCase() === 'h') {
      // Number 1 on vertical line
      tctx.fillText('①', Rx - 30, Cy - 60);
      
      // Number 2 on bump
      tctx.fillText('②', Rx + 30, Cy + 20);
    }
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../g/g.html");
