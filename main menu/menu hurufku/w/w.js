...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // W: stroke order - 4 diagonals creating two V shapes (4 strokes)
    if (currentLetter.toUpperCase() === 'W') {
      // Number 1 on first left
      tctx.fillText('①', Lx - 100, Cy - 140);
      
      // Number 2 on first right
      tctx.fillText('②', Lx - 20, Cy - 140);
      
      // Number 3 on second left
      tctx.fillText('③', Lx + 20, Cy - 140);
      
      // Number 4 on second right
      tctx.fillText('④', Lx + 100, Cy - 140);
    }

    // w: stroke order - left hump, middle, right hump (3 strokes)
    if (currentLetter.toLowerCase() === 'w') {
      // Number 1 on left
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../v/v.html");
