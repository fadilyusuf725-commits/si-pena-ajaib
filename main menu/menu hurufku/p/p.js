...
/* letter / progress */
let currentLetter = 'P';
try { localStorage.setItem('lastVisitedFull', 'menu hurufku/p/p.html'); } catch(e){}
...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // P: stroke order - vertical then upper bump (2 strokes)
    if (currentLetter.toUpperCase() === 'P') {
      // Number 1 on vertical
      tctx.fillText('①', Lx - 60, Cy);
      
      // Number 2 on bump
      tctx.fillText('②', Lx + 40, Cy - 80);
    }

    // p: stroke order - bump then descender (2 strokes)
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../o/o.html");
