...
/* letter / progress */
let currentLetter = 'S';
try { localStorage.setItem('lastVisitedFull', 'menu hurufku/s/s.html'); } catch(e){}
...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // S: stroke order - top curve, bottom curve (2 strokes)
    if (currentLetter.toUpperCase() === 'S') {
      // Number 1 on top curve
      tctx.fillText('①', Lx + 30, Cy - 100);
      
      // Number 2 on bottom curve
      tctx.fillText('②', Lx + 30, Cy + 100);
    }

    // s: stroke order - wavy curve (1 stroke)
    if (currentLetter.toLowerCase() === 's') {
      // Number 1 on curve
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../r/r.html");
