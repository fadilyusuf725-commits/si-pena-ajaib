...
/* letter / progress */
let currentLetter = 'J';
try { localStorage.setItem('lastVisitedFull', 'menu hurufku/j/j.html'); } catch(e){}
...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // J: stroke order - vertical then hook (2 strokes)
    if (currentLetter.toUpperCase() === 'J') {
      // Number 1 on vertical
      tctx.fillText('①', Lx, Cy - 60);
      
      // Number 2 on hook
      tctx.fillText('②', Lx - 50, Cy + 120);
    }

    // j: stroke order - dot then stem-hook (2 strokes)
    if (currentLetter.toLowerCase() === 'j') {
      // Number 1 on dot
      tctx.fillText('①', Rx, Cy - 80);
      
      // Number 2 on stem-hook
      tctx.fillText('②', Rx - 20, Cy + 80);
    }
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../i/i.html");
