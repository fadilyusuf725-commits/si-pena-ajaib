...
/* letter / progress */
let currentLetter = 'Q';
try { localStorage.setItem('lastVisitedFull', 'menu hurufku/q/q.html'); } catch(e){}
...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // Q: stroke order - circle then diagonal tail (2 strokes)
    if (currentLetter.toUpperCase() === 'Q') {
      // Number 1 on circle
      tctx.fillText('①', Lx, Cy - 120);
      
      // Number 2 on tail
      tctx.fillText('②', Lx + 70, Cy + 100);
    }

    // q: stroke order - bump then descender (2 strokes)
    if (currentLetter.toLowerCase() === 'q') {
      // Number 1 on bump
      tctx.fillText('①', Rx + 30, Cy - 60);
      
      // Number 2 on descender
      tctx.fillText('②', Rx + 30, Cy + 80);
    }
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../p/p.html");
