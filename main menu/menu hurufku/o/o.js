...
/* letter / progress */
let currentLetter = 'O';
try { localStorage.setItem('lastVisitedFull', 'menu hurufku/o/o.html'); } catch(e){}
...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // O: stroke order - single circle (1 stroke)
    if (currentLetter.toUpperCase() === 'O') {
      // Number 1 on circle
      tctx.fillText('①', Lx, Cy - 120);
    }

    // o: stroke order - single circle (1 stroke)
    if (currentLetter.toLowerCase() === 'o') {
      // Number 1 on circle
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../n/n.html");
