...
/* letter / progress */
let currentLetter = 'D';
try { localStorage.setItem('lastVisitedFull', 'menu hurufku/d/d.html'); } catch(e){}
...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // D: stroke order - vertical line then bump (2 strokes)
    if (currentLetter.toUpperCase() === 'D') {
      // Number 1 on vertical line
      tctx.fillText('①', Lx - 80, Cy);
      
      // Number 2 on bump
      tctx.fillText('②', Lx + 50, Cy);
    }

    // d: stroke order - bump then vertical line (2 strokes)
    if (currentLetter.toLowerCase() === 'd') {
      // Number 1 on bump
      tctx.fillText('①', Rx + 30, Cy - 60);
      
      // Number 2 on vertical line
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../c/c.html");
