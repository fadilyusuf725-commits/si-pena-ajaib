...
/* letter / progress */
let currentLetter = 'K';
try { localStorage.setItem('lastVisitedFull', 'menu hurufku/k/k.html'); } catch(e){}
...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // K: stroke order - vertical, upper diagonal, lower diagonal (3 strokes)
    if (currentLetter.toUpperCase() === 'K') {
      // Number 1 on vertical
      tctx.fillText('①', Lx - 60, Cy);
      
      // Number 2 on upper diagonal
      tctx.fillText('②', Lx + 50, Cy - 100);
      
      // Number 3 on lower diagonal
      tctx.fillText('③', Lx + 50, Cy + 100);
    }

    // k: stroke order - vertical then diagonal-vertical (2 strokes)
    if (currentLetter.toLowerCase() === 'k') {
      // Number 1 on vertical
      tctx.fillText('①', Rx - 30, Cy);
      
      // Number 2 on diagonal
      tctx.fillText('②', Rx + 40, Cy + 20);
    }
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../j/j.html");
