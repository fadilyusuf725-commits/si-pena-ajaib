...
/* letter / progress */
let currentLetter = 'G';
try { localStorage.setItem('lastVisitedFull', 'menu hurufku/g/g.html'); } catch(e){}
...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // G: stroke order - arc then horizontal (2 strokes)
    if (currentLetter.toUpperCase() === 'G') {
      // Number 1 on arc
    ...
    // g: stroke order - bump then vertical (2 strokes)
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../f/f.html");
