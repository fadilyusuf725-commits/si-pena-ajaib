...
/* letter / progress */
let currentLetter = 'F';
try { localStorage.setItem('lastVisitedFull', 'menu hurufku/f/f.html'); } catch(e){}
...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // F: stroke order - vertical then 2 horizontals (3 strokes)
    if (currentLetter.toUpperCase() === 'F') {
      const topY = Cy - 140;
      const midY = Cy;
    ...
    // f: stroke order (2 strokes)
    if (currentLetter.toLowerCase() === 'f') {
      // Number 1 on curve
      tctx.fillText('①', Rx - 20, Cy - 80);
      
      // Number 2 on vertical line
      tctx.fillText('②', Rx - 20, Cy + 20);
    }
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../e/e.html");
