...
/* letter / progress */
let currentLetter = 'M';
try { localStorage.setItem('lastVisitedFull', 'menu hurufku/m/m.html'); } catch(e){}
...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // M: stroke order - left vertical, right vertical, middle peak (3 strokes)
    if (currentLetter.toUpperCase() === 'M') {
    ...
    // m: stroke order - left hump, middle hump, right hump (3 strokes)
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../l/l.html");
