...
/* letter / progress */
let currentLetter = 'L';
try { localStorage.setItem('lastVisitedFull', 'menu hurufku/l/l.html'); } catch(e){}
...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // L: stroke order - vertical then horizontal (2 strokes)
    if (currentLetter.toUpperCase() === 'L') {
      // Number 1 on vertical
      tctx.fillText('①', Lx - 60, Cy);
      
      // Number 2 on horizontal
      tctx.fillText('②', Lx + 30, Cy + 160);
    }

    // l: stroke order - single vertical stem (1 stroke)
    if (currentLetter.toLowerCase() === 'l') {
      // Number 1 on vertical
      tctx.fillText('①', Rx, Cy);
    }
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../k/k.html");
