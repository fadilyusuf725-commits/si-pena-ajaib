...
/* letter / progress */
let currentLetter = 'X';
try { localStorage.setItem('lastVisitedFull', 'menu hurufku/x/x.html'); } catch(e){}
...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // X: stroke order - left diagonal, right diagonal (2 strokes)
    if (currentLetter.toUpperCase() === 'X') {
      // Number 1 on left-to-right diagonal
      tctx.fillText('①', Lx - 60, Cy - 100);
      
      // Number 2 on right-to-left diagonal
      tctx.fillText('②', Lx + 60, Cy - 100);
    }

    // x: stroke order - left then right diagonal (2 strokes)
    if (currentLetter.toLowerCase() === 'x') {
      // Number 1 on left diagonal
      tctx.fillText('①', Rx - 30, Cy - 40);
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../w/w.html");
