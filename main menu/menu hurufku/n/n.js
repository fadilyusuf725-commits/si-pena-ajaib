...
/* letter / progress */
let currentLetter = 'N';
try { localStorage.setItem('lastVisitedFull', 'menu hurufku/n/n.html'); } catch(e){}
...
function renderGuideTemplate(withArrows = false) {
  ...
  if (withArrows) {
    ...
    // N: stroke order - left vertical, diagonal, right vertical (3 strokes)
    if (currentLetter.toUpperCase() === 'N') {
      ...
      // Number 3 on right vertical
      tctx.fillText('③', Lx + 60, Cy);
    }

    // n: stroke order - vertical then hump (2 strokes)
...
if (backBtn) backBtn.addEventListener('click', ()=> window.location.href = "../m/m.html");
